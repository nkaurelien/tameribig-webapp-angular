...
export function getFormValidationErrorMessage(formGroup: FormGroup): string {
    if (!formGroup.valid) {
        const error: AllValidationErrors = getFormValidationErrors(formGroup.controls).shift();
        if (error) {
            let text;
